...
class IntAdderTreeNode(Testbench):
    def __init__(self, dut) -> None:
        ...
        self.assign_self_params("IN_BITS", "OUT_BITS", "SIGN_EXT", "REGISTER_MIDDLE", "REGISTER_OUTPUT")
...
async def check_pipeline_register_middle_values(dut):
    ...
    exp_ls_adder = unsigned_extend(inputs["a"], dut.LSWidth.value) + unsigned_extend(inputs["b"], dut.LSWidth.value)
    ...
    assert signal_uint(dut.gen_register_middle.cross_carry) == exp_cross_carry, check_msg(
        "check_pipeline_register_middle_values"
    )
    await cc_triggers.FallingEdge(dut.clk)
    assert signal2int(dut.out) == tb.model(**inputs), check_msg("check_pipeline_register_middle_values")
...
async def check_pipeline_registers(dut):
    ...
    if tb.REGISTER_MIDDLE:
        await cc_triggers.FallingEdge(dut.clk)
        assert signal2int(dut.out) == tb.model(**inputs_1), check_msg("check_pipeline_registers")
        await cc_triggers.FallingEdge(dut.clk)
        assert signal2int(dut.out) == tb.model(**inputs_2), check_msg("check_pipeline_registers")
    else:
        assert signal2int(dut.out) == tb.model(**inputs_1), check_msg("check_pipeline_registers")
        await cc_triggers.FallingEdge(dut.clk)
        assert signal2int(dut.out) == tb.model(**inputs_2), check_msg("check_pipeline_registers")
...
def pytest_int_adder_tree_node():
    NUM_RANDOM_TESTS = 5
    param_list = [
        {"IN_BITS": 4, "OUT_BITS": 5, "SIGN_EXT": 0, "REGISTER_MIDDLE": 0, "REGISTER_OUTPUT": 0},
        {"IN_BITS": 4, "OUT_BITS": 5, "SIGN_EXT": 1, "REGISTER_MIDDLE": 0, "REGISTER_OUTPUT": 0},
        {"IN_BITS": 4, "OUT_BITS": 6, "SIGN_EXT": 0, "REGISTER_MIDDLE": 1, "REGISTER_OUTPUT": 0},
        {"IN_BITS": 4, "OUT_BITS": 6, "SIGN_EXT": 1, "REGISTER_MIDDLE": 1, "REGISTER_OUTPUT": 0},
        {"IN_BITS": 4, "OUT_BITS": 6, "SIGN_EXT": 0, "REGISTER_MIDDLE": 1, "REGISTER_OUTPUT": 1},
        {"IN_BITS": 4, "OUT_BITS": 6, "SIGN_EXT": 1, "REGISTER_MIDDLE": 1, "REGISTER_OUTPUT": 1},
    ]
    ...
    lqer_runner(param_list, simulator="icarus")
